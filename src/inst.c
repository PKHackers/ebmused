...
LRESULT CALLBACK InstrumentsWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_CREATE: {
		...
		SendMessage(hwndStatus, SB_SETPARTS, 3, (LPARAM)(int[]){ 200, 400, -1 });
		break;
	}
	case WM_COMMAND: {
		...
		switch (id) {
			...
			case IDC_INSTLIST:
				if (action == LBN_SELCHANGE) {
					...
					format_status(1, "ADSR: %02d/15  %d/7  %d/7  %02d/31", 0xF - (c->inst_adsr1 & 0xF), (c->inst_adsr1 >> 4) & 0x7, (c->inst_adsr2 >> 5) & 7, c->inst_adsr2 & 0x1F);
				}
				break;
		}
		break;
