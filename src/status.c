...
void set_code_tip_status(int p, BYTE *code) {
	if (code[0] == 0x00) {
		...
		switch (code[0]) {
			...
			case 0xE3: format_status(p, "Enable vibrato (delay: %d freq: %d range: %d)", code[1], code[2], code[3]); break;
			...
			case 0xF4: format_status(p, "Finetune %d/256", code[1]); break;
			case 0xF5:
				format_status(p, 
			...
			case 0xF8: format_status(p, "Echo volume (delay: %d lvolume: %d rvolume: %d)", code[1], code[2], code[3]); break;
			case 0xF9: 
